...
```{r}
borrame_df <- data.frame(matrix(ncol = 3, nrow = 0))
colnames(borrame_df) <- c("variety", "blau", "entropy")
...
borrame_df[nrow(borrame_df)+1,] <- diverse::diversity(aux_tbl_var_df, type = c("variety", "blau", "entropy"))
```


```{r}
attributes(cis_2020_df)$variable.labels[colnames(cis_2020_df) == "P11_9"]
attributes(cis_2020_df)$variable.labels[colnames(cis_2020_df) == "P12_9"]
attributes(cis_2020_df)$variable.labels[colnames(cis_2020_df) == "P24_7"]
```
